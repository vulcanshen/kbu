...
func TestStatePath_EnvOverride(t *testing.T) {
	t.Setenv("KM8__STATEPATH", "/tmp/km8-test-state.yaml")
	if got := StatePath(); got != "/tmp/km8-test-state.yaml" {
		t.Errorf("StatePath env override: got %q, want %q", got, "/tmp/km8-test-state.yaml")
	}
}
