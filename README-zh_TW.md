...
- **Compose，不重做** — Alterm（內嵌常駐 shell、`Alt+t`）讓任何你原本得跳出 km8 才能跑的 terminal 工具都能在 km8 裡同居。km8 負責導覽和檢查，需要 write 動作時用你信任的工具 — 沒有 scrollback 切割、沒有 context switch，Alterm 在 `Alt+t` 切換之間 env / cwd / shell history 全程保留
...
- **Alterm 內嵌終端機** — `Alt+t` 在 km8 內切換一個 embedded shell（login shell、完整 env / cwd）— 像在 popup 裡 `ssh localhost`。可以執行 `kubectl apply -f`、`helm`，所有平常你會跳出 km8 才能跑的東西。這個 shell 是**常駐**的：在 popup 顯示中按 `Alt+t` 只會隱藏而不殺 shell；再按一次恢復（cwd、history、env、背景 job 全部保留）。Status bar 右側的 `Alterm` chip 顯示 shell 是否在背景活著。與 `kubectl edit` / `kubectl exec` 獨立 — 你可以同時讓 Alterm 跑著、又在另一個 popup 編輯 resource 或 exec 進 container
...
- **分級 Toast 通知** — info（1 秒、popup-layer 邊框 + `󰵅 km8` 標題、hint 寫 `auto-dismiss`）用於像「Copied!」這類確認；warning（2 秒、Catppuccin Peach 邊框 + `󰀦 km8` 標題）用於被擋掉的動作如 Relatives cycle 偵測、drill 失敗；sticky 版本用在 modal state 契約（drag mode hint）— hint 換成 `Esc: close`、不自動消失
...
| `KM8__ALTERM_SHELL` | 改用這個 binary 作為 Alterm 的 shell。純名字會在 popup 開啟時走 `$PATH` 查找（Go `exec.Command` 語意）、絕對路徑直接 exec。前後空白會被 trim。 | `KM8__ALTERM_SHELL` > `alterm_shell` config > `$SHELL` > `/bin/sh` |
| `KM8__ALTERM_LOGIN_SHELL` | 強制 Alterm shell 進入或退出 login mode（`-l`）。Truthy 值：`true` / `1` / `yes`（大小寫都接受）。其他值關閉 login mode。當從非 login 父 shell 啟動而 PATH 在 `.zprofile` 時使用。 | `KM8__ALTERM_LOGIN_SHELL` > `alterm_login_shell` config > `false` |
