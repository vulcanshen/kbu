# km8 — ZLC Implementation

本文件是 [`zlc-tui-design-principle.md`](./zlc-tui-design-principle.md) 在
km8 上的具體 implementation。通用規範是 **interface**、本文件是 **implementation
class** — 每節對應通用規範同編號的條款、寫 km8 的具體選擇、案例、hotkey 表。

新加入 km8 開發的人讀這份文件、能直接知道「km8 把每條原則 implement 成什
麼樣」。想知道**為什麼**這樣做、回去看通用規範。

本文件**含 popup convention v2 全部 normative 內容**（見 §6）— popup
convention 是 km8 對通用 §6 的完整實作、`one popup = one file = one
PopupAnimator` 的 invariant 在這裡強制執行。

---

## §A. ZLC implementation in km8

通用規範 §A.0（ZLC spectrum 量化框架）+ §A.1（Contextual track）+ §A.2
（Non-contextual track）在 km8 的具體實現。

### §A.0 km8 ZLC score 對照

km8 在通用 §A.0 ZLC spectrum 上的位置：
...
| **Y. core-key role 數量** | 5 | 5 個 distinct role：「focus 切換」(`Tab`) / 「確認」(`Enter`) / 「取消」(`Esc`) / 「contextual 入口」(`Space`) / 「non-contextual 入口」(`?`)。**alias 共用 role 算 1 個**：若 km8 補 `q` 跟 `Esc` 完整 alias 做「取消」（所有 surface 都同樣有效）、仍算 1 個取消 role、Y 不變、仍 = 5。若 `q` 只在某些 surface 取消、其他沒有 — 半套 alias、不算同 role、Y +1 變 6 |
| `min(1, 5/Y)` 係數 | 1.0 | Y = 5、無 penalty |
| **ZLC score** | `1.0 × 1.0 × 100%` = **100%** | user 不靠事先學就能用 |
| 事前認知門檻（反向）| `100% − 100%` = **0** | 不需要看 README |

km8 是「**高 ZLC + Kubernetes 操作工具**」這個 niche 的 prototypical
案例 — 不靠事先學習就能用、靠 Space menu 跟 `?` help popup 走完所有
功能。letter hotkey 是加速捷徑、不是必經之路。

### §A.0.Y km8 core-key 集合（5 個）

| Core-key | km8 語意 | 對應通用條款 |
|---|---|---|
| `Tab` | focus 切換到下個 panel | §4.1 |
| `Enter` | 確認 / drill-down 進入 | §4.1 |
| `Esc` | 取消 / 關閉最上層浮層（km8 §4.3 取消 key）| §4.3 |
...
五個 core-key 的語意在 km8 任意 panel / popup 都不變。

### §A.1 Contextual track in km8 — Space menu

km8 §A.1 入口是 **Space menu**：每個 panel + cursor / popup focus 都接
`Space`、列出當前能做的事。

**Layer 1 — entry key (`Space`) 自身揭露**（強制、通用 §A.1 前提）：km8
...
km8 每個 contextual letter hotkey（`Y` / `E` / `D` / `C` / `S` / `Alt-S` /
...
### §A.2 Non-contextual track in km8 — `?` help popup 入口 + statusbar 揭露

km8 §A.2 入口是 **`?` help popup**：在任何 surface 按 `?` 跳出 app 全
部全域動作的完整清單。

**Layer 1 — entry key (`?`) 自身揭露**（強制、通用 §A.2 前提）：km8 在
...
#### Layer 2 — 個別動作的 ambient 揭露（km8 加碼、通用 §A.2 optional）

Layer 1 是強制揭露 entry key 自身、Layer 2 是 km8 自選加碼 — 在
statusbar 用 chip 持續揭露**個別全域動作**、增加 user 對它們的 ambient
awareness。通用規範不強制、km8 選了做：
...
### km8 contextual / non-contextual 動作邊界（audit 用）
...
| `Esc` | 關閉 / 退出（km8 §4.3 取消 key）| 永遠不當「確認」|
...
km8 在窄寬下：sidebar (panel 1) 可隱藏、panel 2 + panel 3 並列、最窄寬度
仍能瀏覽 + drill-down 資源。

### 1.2 Width stability

km8 panel header 的動態元素（context / namespace 名稱）用 `Lavender` chip
固定 slot；statusbar 的 `[Alt-t]erm` / `[C]ompare` chip 寬度與內容無關
（chip 本身是固定 string、不會因為當前 panel 變寬度）。

### 1.3 Statusbar 行數

**km8 選 N=1**（單行 statusbar）— 通用 §1.3 規定行數一旦選定就鎖死、
km8 一律單行。
...
km8 從 base → Lavender → Sapphire 一路變淺、絕對不逆轉。

### 2.5 km8 layer 插值

km8 採線性 lightness 插值、N=4、預製離散色階：
...
### 2.6 km8 panel-aware brightness dimming

km8 上 `C` 字母同時是 panel 1/3 的 `[C]ontext` picker、跟 panel 2 的
...
km8 假設使用者有 JetBrainsMono Nerd Font 或同等字體、不設計降級分支。
README install 提示安裝、未裝者的使用體驗不在 km8 設計範圍。

### 3.2 km8 glyph 子集

km8 只用 `U+f...` 開頭區段（Font Awesome / Octicons / Material Design
...
見 §A 上面的 core key 表。`Tab` / `Enter` / `Esc` / `Space` 在 km8 任意
surface 都做同一件事。

### 4.2 Letter hotkey ⊆ Space menu

km8 每個 contextual letter hotkey 都在對應 Space menu 出現：
...
### 4.3 km8 取消 key — 選 `Esc`

通用 §4.3 要求指派一個 core key 當「全 app 取消/關閉」；**km8 選 `Esc`**：
...
### 4.4 km8 hotkey discoverability：bracket `[X]label`
...
km8 v1.6 引入 mouse 支援、嚴格走通用規範 §5：
...
## §6. 浮層 in km8 — Popup Convention v2

km8 採 popup convention v2、本節是通用規範 §6 在 km8 的**完整 implementation**。
核心 invariant：
...
Toast 的 text 部分固定為 `km8`（layer-specific glyph：`󰵅 km8` info、
`󰀦 km8` warn）— 每個 toast 讀起來都是「你的 app 在說話」、靠 glyph + border
color 區分等級。
...
km8 把 popup 分四類、每類有固定 layout 規格：
...
| Top padRow | ✓ | ✓ | ✗ | ✗ |
| Bottom padRow | ✓ | ✓ | ✗ | ✗ |
| Body owner | km8 | km8 | km8 | subprocess |
...
- **Frame km8 渲染**（border + title + hint、無 padRow）
...
Body 是 km8 渲染、還是 subprocess 渲染？
├ subprocess → pty
└ km8       → message
```
...
但有些 target 不是 inline、而是**context shift** — user 離開 km8 的 popup
心智模型、進另一個 mode：
...
km8 panel 2 menu 的 `buildPanel2MenuItems` 把 cursor 操作 region 排第一、
panel operation region（Sort 等）排第二、每個 region 有自己的 header。

### §6.6.1 Region 內主要意圖優先（km8 額外規則）

km8 從 v1.7.6 起追加一條 region 內排序規則 — 通用規範未強制、是 km8 自
己的設計選擇：
...
**km8 具體案例**：panel 2 menu 的 `C` action
...
### §6.6.2 Panel border hint 只承載 tab-contextual（km8 額外規則）

通用規範未明示、是 km8 自己的設計選擇 — 跟 §6.6.1 同屬「§6.6.x = km8
...
| `toast.go` | `ToastModel` | transient / sticky、固定 title text 「km8」 |
...
### 7.1 km8 source-target 關係 implementation

通用 §7.1（target 完成後 source 是否還有意義）的 km8 具體實現分兩條路：

| Target 類型 | 是否清除 source | km8 條款 |
|---|---|---|
...
### 7.2 km8 streaming exception：Logs 不退階

km8 panel 3 五個 tab 的 unfocus 處理：
...
| `Enter` | 確認 / drill-down |
| `Esc` | 取消 / 關浮層（km8 §4.3 取消 key） |
| `Space` | Space menu（km8 §A.1 contextual 入口） |
| `?` | help popup（km8 §A.2 non-contextual 入口） |
