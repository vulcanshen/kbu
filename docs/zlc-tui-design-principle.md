...
### 2.6 Hotkey collision disambiguation via brightness

當同一個字母在不同 focus 下綁定不同 action（panel-aware hotkey）、若兩個
指示器**同時可見**、其 bracket+letter 必須 anti-correlated brightness：

- 當前 focus 會觸發的那個：bright（active colour）
- 當前 focus **不**會觸發的那個：dim（inactive / overlay）

亮度的「交班」本身就在傳達「按這個鍵在這個 panel 會打開哪一個」、不需要
文字標籤或 popup 解釋。

成立條件：兩個指示器**同時可見**。如果只有一個會出現、不適用本條（直接用
active colour 即可、沒人會誤會）。

跟 §2.4（warning override 不參與 z-axis）邏輯類似：brightness 在這條被
panel-context 接管、不參與 popup layer scale。

典型案例（km8 v1.7.6）：

| Focus | `[C]ontext` 的 `[C]` | `[C]ompare` 的 `[C]` | 訊息 |
|---|---|---|---|
| Panel 2（table） | dim grey | bright blue | C 在這裡 fires compare、不是 context picker |
| Panel 1 / 3 | bright blue | dim grey | C 在這裡 fires context picker、不是 compare |

讀者掃 statusbar 一眼就知道「我現在按 C 會打開什麼」、不用想 panel 邏輯。
...
### 4.4 Hotkey discoverability — `[X]label` bracket format

任何 surface 上「有 discoverable hotkey 的 label」必須用 bracket 包住熱
鍵字元、整個 app 一個 rule：
...
讀者規則簡化為「**bracket 內就是要按的鍵**」、同條規則跨：

- Statusbar（`[C]ontext: ...`、`[Alt-t]erm` chip）
- Menu entries（`[Y]AML`、`[Alt-S]ort panel 2 list`）
- Popup hints、bottom statusline
- Confirm / picker 的提示行
...
- 用顏色（lavender chip / 特殊 accent）暗示「這是 hotkey 觸發的東西」
  — 顏色 convention 是 designer 心智模型、user 不會自動連起來、要學
- 用 glyph（terminal icon、compare icon）標記「這個 surface 跟某個
  hotkey 相關」— glyph 是類型訊號（§3.3）、不是熱鍵訊號、兼職就破
  §B 元素專職化
- 同 app 內混用 `[X]` / `<X>` / `X:` / 純色強調 多種 hotkey 標記法 —
  user 要學多套規則、ZLC 破洞

**為什麼這條值得寫成原則**：bracket 是跨文化 universal「裡面是按鍵」的
直覺（vim help、cli help、man page 都這樣標）— 不靠教學記得住。顏色 /
glyph 是 designer 心理模型、跨 user 不普及。

跟 §2.6（hotkey collision via brightness）配合：bracket 標「這是鍵」、
brightness 標「這鍵在這 focus 會不會 fire」。
...
#### 6.6.1 Region 內的 item 順序：context-aware 主要意圖優先

§6.6 規範 region 之間的順序、本條規範 region 內 item 的順序。

當 menu 開啟時的 app state 暗示 user **此次開 menu 的主要意圖**、該 item
必須提升到所屬 region 的第一順位、即使它在「無 context」狀態下原本排在
後面。

**判準**：

> 「user 在這個 state 下開 menu、99% 會想做哪一件事？」
> 那一件事浮頂、其他 item 維持原序。

**提升要保守**：只有 state 明確指向**單一**意圖時提升、否則維持原序。
否則每次都洗牌、user 的 muscle memory 失效（同一個 menu、每次 item 0
都不同 action、user 反而要每次低頭讀標籤）。

| 應提升 | 不應提升 |
|---|---|
| State 明確、99% 用途指向單一 item | State 模糊、多個 item 都合理 |
| Item 在無 context 時排序靠後（不會誤升） | Item 已經是 region 第一順位（提升無意義） |
| 提升的 item 跟其他 item 是同類動作（同 region） | 跨 region 提升（破 §6.6 region 分組） |

**範例**（km8 v1.7.6 panel 2 menu C action）：

| Menu 開啟時 state | 主要意圖 | 提升項 |
|---|---|---|
| In compare mode、cursor 在 candidate row | 「比對該 row 跟 anchor」幾乎是 100% 意圖（user 設好 anchor、移到別 row、Space 開 menu — 就是要 diff） | "Compare to anchor" → item 0 |
| In compare mode、cursor 在 anchor 上 | 不是「來這做事」、是 cleanup verb | "Unmark" 維持原序 |
| 未 lock | 還沒設定 anchor、user 可能要做任何 row action | "Mark" 維持原序 |

**反例**（不該提升）：

- 「panel 2 cursor 在某 pod row」→ 不提升 `[Y]AML`、因為 user 可能要 Edit / Shell / Delete / YAML / drill、意圖不單一
- 「user 剛用過 Edit」→ 不提升 Edit、過去用過不等於現在還要

**跟 §6.6 的關係**：§6.6 保證 region 順序固定（muscle memory）、本條只在
**region 內**重排、且僅當 state 強訊號時。Region 之間的順序永遠穩定、
只有 region 內最頂端的單一 item 可能變、整體洗牌幅度受限。
