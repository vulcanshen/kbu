...
Toast 的 text 部分固定為 `KubeUI`（layer-specific glyph：`󰵅 KubeUI` info、
`󰀦 KubeUI` warn）— 每個 toast 讀起來都是「你的 app 在說話」、靠 glyph + border
color 區分等級。
...
| `toast.go` | `ToastModel` | transient / sticky、固定 title text 「KubeUI」 |
