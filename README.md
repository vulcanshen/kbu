...
- **Session state persistence (v1.7.10)** -- quit and relaunch drops you back where you were. km8 records the current `(context, namespace, kind, panel-2 row cursor, focused panel)` on every quit to a `state.yaml` file sitting alongside `config.yaml` in the config directory. Next launch applies the recorded context + namespace before the k8s client connects, restores the sidebar cursor to the recorded Kind, and snaps the panel-2 cursor onto the last-selected object once its watcher tick arrives. Focused panel (sidebar / table / detail) is restored too, so `q` from panel 2 lands you back on panel 2. Recorded values that no longer exist (namespace deleted, CRD uninstalled, object churned away) fall back to defaults with an INFO line in the App Log — no toast, no warning; you'll notice via `!` if you care. `state.yaml` is kept separate from `config.yaml` on purpose: config is your hand-authored preferences (comments preserved), state is auto-rewritten every quit — mixing them would break the "config is my document" trust boundary
